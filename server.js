...
const secretClient = new SecretManagerServiceClient({projectId: 'alt-text-org'});
...
async function getClients() {
    const twitterClientId = await getSecret("projects/alt-text-org/secrets/twitter-client-id/versions/1")
    const twitterClientSecret = await getSecret("projects/alt-text-org/secrets/twitter-client-secret/versions/1")
    const twitterAuth = makeTwitterAuthClient(twitterClientId, twitterClientSecret);

    const pineconeApiKey = await getSecret("projects/alt-text-org/secrets/pinecone-api-key/versions/1")
    const pinecone = makePineconeClient(pineconeApiKey);

    const credEncryptionKey = getSecret("projects/alt-text-org/secrets/twitter-auth-encryption-key/versions/1")
    const firestore = makeFirestoreClient(getFirestore(), credEncryptionKey, "alt-text", "authors", "twitter-auth");

    const storage = makeStorage(pinecone, firestore);

    return {twitterAuth, pinecone, firestore, storage}
}
...
(async () => {
    ...
    const saveOpts = {
        schema: {
            ...
            getSearchables(image.base64, image.url).then(async (searchables) => {
                ...
                const success = await clients.storage.saveAlt(searchables.sha256, searchables.goldberg544, image.url, language, alt_text, id_scope, author_id);
                if (success) {
    ...
    const reportOpts = {
        schema: {
            ...
            const alt = await clients.firestore.getSpecificAlt(author_uuid, sha256, language);
            if (!alt) {
    ...
    const signupOpts = {
        handler: (request, reply) => {
            ...
            const authUrl = clients.twitterAuth.generateAuthURL({
    ...
    const signupCallbackOpts = {
        schema: {
            ...
            try {
                ...
                delete twitterStates[state];
                const token = await clients.twitterAuth.requestAccessToken(code);
                if (token && token.token && token.token.access_token) {
                    ...
                    if (user) {
                        await clients.firestore.saveCredentials(user.data.id, token.token.access_token, token.token.refresh_token, token.token.expires_at);
