...
const { makePineconeClient } = require("./pinecone");
const { makeFirestoreClient } = require("./firestore");
const { makeStorage } = require("./storage");
const { ocrUrl, ocrBase64 } = require("./ocr");
const {
  makeTwitterAuthClient,
  makeTwitterUserClient,
  TWITTER_STATE,
} = require("./twitter");
const { searchablesForUrl, searchablesForBase64 } = require("./searchables");
...
const twitterAuth = makeTwitterAuthClient(
  config.twitter.clientId,
  config.twitter.clientSecret
);

const pinecone = makePineconeClient(config.pineconeApiKey);
const firestore = makeFirestoreClient(
  getFirestore(),
  config.twitter.credEncryptionKey,
  "alt-text",
  "authors",
  "twitter-auth"
);
  ...
  handler: (request, reply) => {
    ...
    getSearchables(image.base64, image.url)
      .then((searchables) => {
        if (!searchables) {
          console.log(
            `${ts()}: Failed to get searchables for ${image.url || "<base64>"}`
          );
          reply
            .status(400)
      ...
      .catch((err) => {
        console.log(
          `${ts()}: Failed to get alt text for ${image.url || "<base64>"}`
        );
        console.log(err);
...
const saveOpts = {
  schema: {
    body: {
      type: "object",
      required: ["image", "language", "alt_text", "id_scope", "author_id"],
      properties: {
        image: {
          type: "object",
          properties: {
  ...
  handler: (request, reply) => {
    const { searchables, image, language, alt_text, id_scope, author_id } =
      request.body;
    ...
    getSearchables(image.base64, image.url)
      .then((searchables) => {
        ...
        if (success) {
          console.log(
            `${ts()}: Successfully saved alt text for ${image.url || "<base64>"}`
          );
          reply.status(204).send();
        } else {
          console.log(
            `${ts()}: Failed to save alt text for ${image.url || "<base64>"}`
          );
          reply.status(500).send({ error: "Internal server error" });
        }
      })
      .catch((err) => {
        console.log(
          `${ts()}: Error saving alt text for ${image.url || "<base64>"}`
        );
        console.log(err);
        reply.status(500).send({ error: "Internal server error" });
      });
...
const reportOpts = {
  schema: {
    body: {
      type: "object",
      required: ["author_uuid", "sha256", "language", "reason"],
      properties: {
        reason: { type: "string" },
  ...
  handler: async (request, reply) => {
    ...
    if (!alt) {
      console.log(
        `${ts()}: Got report for a non-existant alt text at author: '${author_uuid}' lang: ${language} sha: ${sha256}`
      );
...
const signupOpts = {
  handler: (request, reply) => {
    ...
    const authUrl = twitterAuth.generateAuthURL({
      ...
      code_challenge: crypto.randomBytes(64).toString("base64"),
    });
  ...
  handler: async (request, reply) => {
    try {
      ...
      if (token && token.token && token.token.access_token) {
        ...
        if (user) {
          await firestore.saveCredentials(
            user.data.id,
          ...
          console.log(
            `${ts()}: Saved credentials for user ${user.data.id}: @${
              user.data.username
            }`
          );
